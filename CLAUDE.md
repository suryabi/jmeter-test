...
| npm | 10+ |
| Java | **JMeter only:** 11+ (17+ recommended on Windows). System `JAVA_HOME` may stay on **Java 8** for backend — use `JMETER_JAVA_HOME` in `.env` |
| JMeter | 5.4+ with **jpgc-json** plugins |
| Network | HTTPS to BriefingIQ host; valid Bearer in plan/props |

```bash
npm run init                  # first time
npm run setup:jmeter-java     # detect JDK 11+ → JMETER_JAVA_HOME in .env
npm run install:jmeter-java   # or download portable Temurin 17 into .jdk/
npm run dev                   # API :5050 + UI :4200
```

Full docs: `README.md`. Copy `.env.example` → `.env` for `JMETER_HOME` / `JMETER_JAVA_HOME`.
...
scripts/validate.js             # prerequisites + Java resolution for JMeter
scripts/setup-jmeter-java.js    # configure JMETER_JAVA_HOME in .env
scripts/install-portable-jmeter-java.js  # download Temurin into .jdk/
ui/                             # Angular app
runs/                           # gitignored — per-run logs, JTL, reports
.jdk/                           # gitignored — portable Temurin (install:jmeter-java)
.env                            # gitignored — JMETER_HOME, JMETER_JAVA_HOME
...
2. Read `run.meta.json`, `launcher.log`, `jmeter.log`, `result.jtl`
3. Distinguish compile failures (never started) vs runtime/API failures vs **JVM crash** (Windows Java 8)
4. Fix root cause in `plans/*.jmx`, backend/UI, or local Java/JMeter config as appropriate
...
- **JMeter Java** (does not change system `JAVA_HOME`):
  - `JMETER_JAVA_HOME` in `.env` — highest priority
  - Then `jmeter.bat` / `setenv.bat`, then system `JAVA_HOME`
  - On Windows with Java 8 only, auto-discover newer 64-bit JDK under `Program Files`, or `.jdk/temurin-17/`
  - Setup: `npm run setup:jmeter-java` or `npm run install:jmeter-java`
  - Resolution logic: `scripts/validate.js` → `resolveJavaHomeForJmeter()`
  - Launches via `java -jar ApacheJMeter.jar` when `JMETER_HOME` is set (`server.js` → `buildJmeterLaunch()`)
- Restart Node after `jmx-parameters.js` or `.env` changes
...
|---------|----------------|
| JVM crash / exit `3221225477` on Windows | Java 8 used for JMeter — run `npm run install:jmeter-java`; check `launcher.log` for `jdk-1.8` |
...
| jpgc-json compile error | Run `npm run install:jmeter-plugins` |
